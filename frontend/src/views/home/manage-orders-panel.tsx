...
import { RiArrowRightLine, RiRefund2Line, RiMoneyDollarCircleLine } from "react-icons/ri";
import abi from "../../assets/json/abi";
import { CONTRACT_ADDRESS, U128_MAX, U64_MAX } from "../../utils/constants";
import { btnPrimary, inputStyle } from "./shared";

type ActionMode = "withdraw_strk" | "withdraw_wbtc" | "refund_wbtc" | "refund_strk";
...
export default function ManageOrdersPanel() {
  ...
  const handleWithdrawStrk = async () => {
    ...
    try {
      ...
      toast.success("STRK claimed! Your secret is now public — Bob can claim wBTC.");
  ...
  const MODES: { key: ActionMode; label: string; sublabel: string; icon: React.ReactNode }[] = [
    { key: "withdraw_strk", label: "Claim STRK", sublabel: "Alice reveals secret", icon: <RiMoneyDollarCircleLine size={13} /> },
    { key: "withdraw_wbtc", label: "Claim wBTC", sublabel: "Bob uses revealed secret", icon: <RiArrowRightLine size={13} /> },
    { key: "refund_wbtc", label: "Refund wBTC", sublabel: "Alice, if order expired", icon: <RiRefund2Line size={13} /> },
    { key: "refund_strk", label: "Refund STRK", sublabel: "Bob, if Alice vanished", icon: <RiRefund2Line size={13} /> },
  ];
  ...
  const canAct = () => {
    ...
    if (mode === "refund_wbtc") return orderStatus.isExpired && !orderStatus.swapInitiated && !orderStatus.isFilled;
  ...
  const getStatusMessage = () => {
    ...
    if (orderStatus.isWithdrawn) return { text: "Already claimed.", color: "#22c55e" };
    if (orderStatus.isRefunded) return { text: "Already refunded.", color: "#22c55e" };
    if (mode === "withdraw_strk" && orderStatus.isExpired) return { text: "STRK order expired — use Refund STRK instead.", color: "#f87171" };
    if (mode === "withdraw_wbtc" && !orderStatus.secretRevealed) return { text: "Secret not yet revealed. Wait for Alice to call withdraw_strk first.", color: "#f59e0b" };
    if (mode === "withdraw_wbtc" && orderStatus.secretRevealed) return { text: "Secret is on-chain! You can claim your wBTC.", color: "#22c55e" };
    if (mode === "refund_wbtc" && orderStatus.isFilled) return { text: "Order is filled — cannot refund while Bob's STRK is locked.", color: "#f87171" };
    if (mode === "refund_wbtc" && !orderStatus.isExpired) return { text: "Order hasn't expired yet.", color: "#f59e0b" };
    if (mode === "refund_strk" && !orderStatus.isExpired) return { text: "Order hasn't expired yet.", color: "#f59e0b" };
    return null;
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
        {MODES.map(({ key, label, sublabel, icon }) => (
          <button
            key={key}
            onClick={() => { setMode(key); setOrderStatus(null); }}
            style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", color: mode === key ? "#ffc800" : "#555", fontSize: "0.72rem", fontWeight: 700, marginBottom: "0.2rem" }}>
              {icon} {label}
            </div>
            <div style={{ color: "#2a2a3a", fontSize: "0.58rem", letterSpacing: "0.08em" }}>{sublabel}</div>
          </button>
        ))}
      </div>
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          <button
            ...
            style={{
              ...
              width: "auto", padding: "0.6rem 1rem", flexShrink: 0,
            }}>
            {lookupLoading ? <FaSpinner size={11} style={{ animation: "spin 1s linear infinite" }} /> : "Check"}
          </button>
        </div>

        {orderStatus && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", background: "#0a0a0f", borderRadius: 8, padding: "0.85rem" }}>
            <StatusRow label="Withdrawn" value={orderStatus.isWithdrawn ? "Yes" : "No"} bad={orderStatus.isWithdrawn} />
            <StatusRow label="Refunded" value={orderStatus.isRefunded ? "Yes" : "No"} bad={orderStatus.isRefunded} />
            {orderStatus.expiry && (
              <StatusRow
                label="Expiry"
            ...
            {orderStatus.strkAmount && (
              <StatusRow label="STRK amount" value={(Number(orderStatus.strkAmount) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 4 }) + " STRK"} />
            )}
            {orderStatus.wbtcAmount && (
              <StatusRow label="wBTC amount" value={orderStatus.wbtcAmount + " sat"} />
            )}
          </div>
        )}

        {statusMsg && (
          <div style={{ color: statusMsg.color, fontSize: "0.68rem", padding: "0.6rem", background: statusMsg.color + "10", borderRadius: 6, lineHeight: 1.6 }}>
            {statusMsg.text}
          </div>
        )}
      </div>
      ...
      {mode === "withdraw_strk" && (
        <div style={section}>
          ...
          <input
            ...
            style={inputStyle}
          />
          <label htmlFor="secret-file" style={uploadLabel}
            onMouseEnter={(e) => (e.currentTarget.style.borderColor = "#555")}
            onMouseLeave={(e) => (e.currentTarget.style.borderColor = "#1e1e2e")}>
            <FaUpload size={10} /> Upload umbra-swap-secret.json
            <input id="secret-file" type="file" accept=".json" style={{ display: "none" }}
              onChange={(e) => { const f = e.target.files?.[0]; if (f) loadSecretFromFile(f); }} />
          </label>
          {secret && (
            <p style={{ color: "#3a3a4a", fontSize: "0.62rem", margin: 0, wordBreak: "break-all" }}>
      ...
      {!account && (
        <p style={{ color: "#f59e0b", fontSize: "0.65rem", textAlign: "center", margin: 0 }}>
...
function HintBox({ mode }: { mode: ActionMode }) {
  ...
  const h = hints[mode];
  return (
    <div style={{ background: "rgba(255,200,0,0.02)", border: "1px solid #1e1e2e", borderRadius: 8, padding: "0.85rem 1rem" }}>
      <div style={{ color: "#555", fontSize: "0.65rem", fontWeight: 700, marginBottom: "0.25rem" }}>{h.title}</div>
      <div style={{ color: "#3a3a4a", fontSize: "0.62rem", lineHeight: 1.8 }}>{h.body}</div>
    </div>
  );
}

function StatusRow({ label, value, good, bad }: { label: string; value: string; good?: boolean; bad?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between" }}>
      <span style={{ color: "#3a3a4a", fontSize: "0.62rem" }}>{label}</span>
      <span style={{ color: good ? "#22c55e" : bad ? "#f87171" : "#aaa", fontSize: "0.62rem" }}>{value}</span>
    </div>
  );
}
...
const sectionLabel: React.CSSProperties = {
  color: "#3a3a4a", fontSize: "0.6rem", letterSpacing: "0.15em", textTransform: "uppercase",
};
const uploadLabel: React.CSSProperties = {
  display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem",
  padding: "0.65rem", border: "1px dashed #1e1e2e", borderRadius: 8,
  color: "#3a3a4a", fontSize: "0.68rem", cursor: "pointer",
