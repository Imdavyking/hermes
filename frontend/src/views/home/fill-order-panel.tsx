...
export default function FillOrderPanel() {
  ...
  const [fillStep, setFillStep] = useState<"idle" | "approve" | "fill" | "done">("idle");
    ...
    try {
      ...
      for (const e of events.events) {
        try {
          ...
          const orderId = "0x" + ((BigInt(orderIdHigh) << 128n) | BigInt(orderIdLow)).toString(16);
          ...
          if (
            ...
            parsed.push({
              ...
              aliceStrkDest: "0x" + BigInt(orderData.alice_strk_destination).toString(16),
  ...
  const lookupOrder = async () => {
    ...
    try {
      ...
      const now = Math.floor(Date.now() / 1000);
      if (Number(orderData.expiry) < now) return toast.error("Order has expired.");
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const currentAllowance = uint256.uint256ToBN({ low: allowanceResult[0], high: allowanceResult[1] });
  ...
  const handleFill = async () => {
    ...
    try {
      ...
      const tx = await account.execute([
        ...
        maxFee: U128_MAX,
        resourceBounds: {
          l1_gas: { max_amount: "0x100000", max_price_per_unit: U128_MAX },
          l1_data_gas: { max_amount: "0x100000", max_price_per_unit: U128_MAX },
      ...
      await account.waitForTransaction(tx.transaction_hash);
      toast.success("Order filled! Alice will reveal the secret — then claim your wBTC.");
      setFillStep("done");
    } catch (err: any) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      {/* Info */}
      <div style={infoBox}>
        <RiExchangeLine size={14} color="#ffc800" style={{ flexShrink: 0, marginTop: 2 }} />
        <div style={{ fontSize: "0.68rem", color: "#555", lineHeight: 1.8 }}>
          Browse open wBTC orders. Fill one by locking STRK. Once Alice reveals her secret,
          use <b style={{ color: "#aaa" }}>Manage Orders</b> to claim your wBTC.
        </div>
      </div>
      ...
      <div style={section}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={fetchOrders} disabled={loadingOrders} style={{ ...btnGhost, width: "auto", padding: "0.3rem 0.75rem", fontSize: "0.65rem" }}>
            {loadingOrders ? <FaSpinner size={10} style={{ animation: "spin 1s linear infinite" }} /> : <FaSync size={10} />}
            &nbsp;Refresh
          </button>
        </div>

        {loadingOrders ? (
          <div style={{ textAlign: "center", color: "#3a3a4a", padding: "1.5rem", fontSize: "0.72rem" }}>
            <FaSpinner size={18} style={{ animation: "spin 1s linear infinite", marginBottom: "0.5rem", display: "block", margin: "0 auto 0.5rem" }} />
            Scanning on-chain events…
          </div>
        ) : orders.length === 0 ? (
          <div style={{ textAlign: "center", color: "#2a2a3a", padding: "1.5rem", fontSize: "0.72rem", letterSpacing: "0.08em" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            {orders.map((o) => (
              <OrderCard
                ...
                onSelect={() => { setSelectedOrder(o); setFillStep("approve"); }}
              />
            ))}
          </div>
        )}
      ...
      <div style={section}>
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          />
          <button onClick={lookupOrder} style={{ ...btnGhost, width: "auto", padding: "0.6rem 1rem", flexShrink: 0 }}>
            <FaSearch size={11} />
          </button>
        </div>
      </div>
      ...
      {selectedOrder && fillStep !== "done" && (
        <div style={{ ...section, borderColor: "rgba(255,200,0,0.25)", background: "rgba(255,200,0,0.03)" }}>
          <div style={sectionLabel}>Selected order</div>
          <div style={detailGrid}>
            <Detail label="Order ID" value={selectedOrder.orderId.slice(0, 18) + "…"} />
            <Detail label="wBTC amount" value={Number(selectedOrder.wbtcAmount).toLocaleString() + " sat"} />
            <Detail label="STRK you lock" value={(Number(selectedOrder.quotedStrkAmount) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 2 }) + " STRK"} highlight />
            <Detail label="Expires" value={new Date(selectedOrder.expiry * 1000).toLocaleTimeString()} />
          </div>

          {/* Bob expiry */}
          <div style={sectionLabel}>Your expiry (must be &lt; Alice's)</div>
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            {EXPIRY_PRESETS.filter(p => p.secs < (selectedOrder.expiry - now)).map(({ label, secs }) => (
              <button key={secs} onClick={() => setBobExpirySeconds(secs)} style={chipStyle(bobExpirySeconds === secs)}>
                {label}
              </button>
            ))}
          </div>
          <p style={{ color: "#3a3a4a", fontSize: "0.62rem", margin: 0 }}>
            <RiTimeLine style={{ verticalAlign: "middle" }} /> Your STRK is locked for {bobExpirySeconds / 3600}h. If Alice doesn't reveal in time, refund it.
          </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.6rem" }}>
            <button
              ...
              {approveLoading ? <FaSpinner size={12} style={{ animation: "spin 1s linear infinite" }} /> : null}
            ...
            <button
              ...
              {fillLoading ? <FaSpinner size={12} style={{ animation: "spin 1s linear infinite" }} /> : null}
              2. Fill Order
            </button>
          </div>
        </div>
      ...
      {fillStep === "done" && (
        <div style={{ ...infoBox, background: "rgba(255,200,0,0.06)", borderColor: "rgba(255,200,0,0.25)", padding: "1.5rem", flexDirection: "column" }}>
          <div style={{ color: "#ffc800", fontWeight: 900, fontSize: "0.9rem" }}>🎉 Order filled!</div>
          <div style={{ color: "#555", fontSize: "0.68rem", marginTop: "0.4rem", lineHeight: 1.7 }}>
            Your STRK is locked. Alice will reveal the secret to claim her STRK.
            Once the secret is public, go to <b style={{ color: "#aaa" }}>Manage Orders</b> and paste your STRK order ID to claim your wBTC.
          </div>
        </div>
      )}
    </div>
...
function OrderCard({ order, now, selected, onSelect }: {
  order: OpenOrder; now: number; selected: boolean; onSelect: () => void;
}) {
  ...
  return (
    <button
      ...
      <div>
        <div style={{ color: "#555", fontSize: "0.62rem", marginBottom: "0.2rem" }}>
      ...
      <div style={{ textAlign: "right" }}>
        <div style={{ color: "#ffc800", fontSize: "0.82rem", fontWeight: 700 }}>
          {(Number(order.quotedStrkAmount) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 0 })} STRK
        </div>
        <div style={{ color: rateExpired ? "#f87171" : "#3a3a4a", fontSize: "0.6rem", marginTop: "0.2rem" }}>
...
function Detail({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between" }}>
      ...
      <span style={{ color: highlight ? "#ffc800" : "#aaa", fontSize: "0.62rem", fontFamily: "'DM Mono', monospace" }}>{value}</span>
    </div>
  );
}
...
const detailGrid: React.CSSProperties = {
  ...
  background: "#0a0a0f", borderRadius: 8, padding: "0.8rem",
};
...
const chipStyle = (active: boolean): React.CSSProperties => ({
  ...
  borderRadius: 6, padding: "0.35rem 0.75rem", fontSize: "0.7rem",
  fontFamily: "'DM Mono', monospace", fontWeight: active ? 900 : 400,
